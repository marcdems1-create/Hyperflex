...
**✅ LIVE — trader surface public as of 2026-07-23, ALONGSIDE home.html, not replacing it.** Provisional banners, `provisional`/`provisional_note` flags, and the per-card provisional badge are all removed; `/traders` is a real Express route serving the trader-first surface, linked prominently from site nav (`Traders`, first primary link). `home.html` is untouched and still serves `/` — this was Marc's explicit call, not an oversight: the trader page is structurally right per this product definition but visually unfinished (a design pass was deliberately deferred), and swapping the homepage before that pass would make the least-polished surface the front door. Running both surfaces lets `/traders` be observed under real traffic first. Trader-first is still the destination and `home.html`'s market grid is still on borrowed time — the swap happens after the design pass, not before it. See SESSION_STATE.md 2026-07-23 for the full record.
