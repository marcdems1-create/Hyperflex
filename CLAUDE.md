...
**⚠️ Found while building this, not previously flagged here: a MORE automated copy system already exists** — `public/copy-bot.js` (auto-loaded on every page via `nav.js`), `public/copy-trading.html`, and `/api/copy-bot/{subscribe,pending,stream,history,...}` in `server.js`. It lets a user subscribe to a whale and get trades pushed via SSE, with an attempt at background execution using the user's own stored CLOB API keys + browser-wallet signature (still non-custodial — no raw private key is stored, `HFXWallet.getSigner()` still needs a live wallet signature per order — but the trade's side/size/market is chosen by the subscription rule, not reviewed per trade by the user the way this Gate 4 SAFE-tier build requires). This existed before the 2026-07-30 build and predates this Gate being written down; it was not built or modified as part of this work. Decision on whether it should be kept, gated, or folded into Gate 4's discipline is still open — flagged, not resolved.
