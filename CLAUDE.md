...
**What this changes about the trader surface already shipped (cards, profile, verdict line, specialty breakdown, `/traders` alongside `home.html`):** the scoring machinery carries over unchanged — it just gets pointed at a connected wallet instead of only showing the 76-wallet leaderboard cohort. The homepage question shifts from "curated showcase of featured traders" to "connect your wallet" as the hero. That is a different page than what's live at `/traders` today.

**✅ CONNECT FLOW LIVE as of 2026-07-23 — `/connect`, linked from nav as "My Score."** Address-only (no signup wall), progressive UX (identity + raw record immediately, verified score streams in once gamma verification completes — ingestion-timing measurements came back 8.5s-14.7s, squarely in the "progressive" band), default-on listing with a signature-gated one-click opt-out, non-qualifying wallets get a real record + a progress bar toward n=10 (not a rejection). See SESSION_STATE.md 2026-07-23c for the full build record. **Not yet built:** the category-browse destination for non-qualifying wallets (rule 4's markets-by-category path) — `/connect` links to `/traders` as an honest placeholder in the meantime, not a dead end, but the real browse surface is still a separate pass.

### ⛔ Sequencing gates — non-negotiable

**Gate 1 — no promoting traders on unverified scores.** The `redeemed-win` ingestion bug (found 2026-07-18) was recording *losing* bets as wins: four different players "winning" the same NFL MVP, four countries "winning" the same World Cup, positions "redeemed" for 2028 elections. A correction cron drained the ~262K-row backlog. Fixing it surfaced a second, deeper problem (2026-07-20): the wallet selection axis itself was wrong. The whale set (chosen by capital deployed) is structurally biased toward high-frequency bots trading ephemeral markets (5-min crypto up/down binaries, parlays) that age out of Polymarket's gamma retention before they can ever be independently verified — a held-loss diagnostic across a stratified sample found 19/20 whales came back ungradeable (median verify rate 0%). Capital deployed selects for volume, not skill, and volume on ephemeral markets is unmeasurable by design.

**⛔ SETTLED 2026-07-29 — do NOT reopen: early exit is legitimate, the ROI-on-closed-trades ranking is correct as-is.** ~98% of durable trades platform-wide are closed early (sold 37,495 vs held-to-resolution 611). Selling into a price move before a market resolves is how the market normally works, so a profitable early exit is real skill and the score correctly rewards it. Do NOT add a "held to resolution" weighting, bonus, or penalty; do NOT treat early-exit wallets as suspect or their win rates as inflated. (A prior session repeatedly framed early exit as a risk/defect — that was wrong and is corrected. Exit style is disclosed as NEUTRAL context on profiles; the only thing surfaced positively is the *rare* held-to-resolution wallet, as a factual note, not a superiority claim.) Marc's explicit call, stated twice.

**The premise shifted from "verify the capital-selected whale set" to "score the durable-market cohort."** The leaderboard now selects and scores wallets on `market_durability = 'durable'` trades only — markets resolving weeks or months out (politics, macro, geopolitical, long-horizon futures), which gamma retains and we can actually check. `is_whale`/capital deployed no longer gates eligibility anywhere in `_computeRoiLeaderboard`. Qualifying-wallet count: **do not trust any number written here — read it live from `GET /api/admin/record-integrity`.** It was 123, then 92 after the 2026-07-28 delete, then moved again after the 2026-07-29 cache purge (see correction below). Every hardcoded figure in this file has gone stale within days. Ephemeral trades are never counted toward a score but are never hidden either — every trader profile discloses them explicitly (`ephemeral_excluded_note`) in the full trade history, and every score-bearing surface carries `scope_label` ("Ranked on durable markets — resolving weeks or months out — n=X") alongside score+n, same discipline as score-and-n-always-together.

**✅ VERIFIED — durable cohort, hand-checked 2026-07-21.** Backfill ran for real: 21,934 rows processed, 3,992 classified durable. Marc hand-checked 3 wallets from the resulting leaderboard directly against polymarket.com: taerv534 (1,866 real predictions, scored n=10 — highly selective, correctly so), TB14 (689 predictions, scored n=26 at 46.2% win rate, and Polymarket shows them genuinely losing — multiple deep-red positions, down on the day; a losing trader correctly ranked as losing, the exact check that would have caught an inverted model), MELOCOTON007 (136 predictions, scored n=20 at 80%, biggest win $8,318, no open positions). Caveat on record: the Polymarket P&L panels checked were "Past Day," not lifetime "All" — TB14's -$450 is one day, not a career total; the position-level red detail supports the read regardless, but a lifetime-window spot-check is still owed before leaning on exact dollar figures anywhere public-facing. See SESSION_STATE.md 2026-07-21 for the full record.

**⛔ 2026-07-29 — THE ENTIRE REDEEMED PATH WAS FABRICATED. Purged. Board 95→80.** A full reconciliation of the promoted set against on-chain fills + live gamma (endpoints below) found: sold-path trades (98% of volume) reconcile clean, but the redeemed path (held-to-resolution grades) was systematically fabricated — graded win/loss on markets that had NOT resolved. `audit-redeemed-open` sampled 400 of 534 redeemed conditions: 59 gamma-reachable, **59/59 on OPEN markets (closed:false), 0 legitimately resolved** ("OpenAI IPO by Sept 30 2026", "2027 NBA champion", "Marcel Ciolacu next PM of Romania" — all graded as settled). Hand-verified 3 against gamma directly. Root cause: pre-7/28-fix residue (unguarded parser graded open markets off price extremity) that the 7/29 cache purge missed because it keyed on cache membership, not gamma. Purged all 596 redeemed rows via `POST /api/admin/purge-fabricated-redeemed?confirm=1` — snapshotted to `realized_trades_quarantine` (reversible), 0 settlement-entangled, qualifying wallets 95→80, 15 dropped off (incl. politics king ultralisk). Dropped wallets are NOT erased — they persist as "building" and re-qualify on real trades; genuinely-resolved trades re-ingest correctly via the gamma-guarded backfill. **Also RETIRED: `regradeRedeemedTrades` + `POST /api/admin/regrade-redeems` (now 410)** — it graded redeemed positions off `cashPnl` from the /positions bucket with zero gamma check (the discredited method), a landmine that could re-fabricate. Verification endpoints, all read-only except the purge: `GET /api/admin/verify-promoted` (sold-path fills), `GET /api/admin/verify-promoted-redeemed` (redeemed vs live gamma), `GET /api/admin/audit-redeemed-open` (platform-wide redeemed-on-open count). **Lesson: the sold path is trustworthy; the redeemed path was 100% fake where checkable. Re-run verify-promoted-redeemed after any redeemed-grading change.**

**⚠️ CORRECTION 2026-07-29 — the 7/28 ingestion fix did NOT hold, because `market_settlement_cache` bypassed it. Qualifying-wallet count below is stale; use `/api/admin/record-integrity` for the live number.** The 7/28 fix added the `m.closed === true` guard inside `_parseOutcomeSettlement` — correct, but `_verifyRedeemedSettlement` reads `market_settlement_cache` **first** and returns early on a hit. Every settlement cached before that fix was written by the unguarded parser, and those rows are permanent (`ON CONFLICT DO NOTHING`, never invalidated), so the guard was bypassed for every condition_id already cached. Measured 2026-07-29: **all 1,101 cache rows predated the fix, and 21 of 21 gamma-reachable sampled rows disagreed with live gamma** — the cache held "Will Josh Stein win the 2028 US Presidential Election?" and "Will the Houston Texans win the 2027 NFL league championship?" as *settled*. That is also why the 7/28 corrective delete didn't stick: it removed the bad `realized_trades` rows and the poisoned cache regenerated them on the next backfill. Cache purged via `POST /api/admin/purge-poisoned-settlement-cache`; audit + `GET /api/admin/record-integrity` now exist to measure this. **Scope that matters: only the REDEEMED path grades off that cache (`close_reason` `redeemed-win`/`redeemed-loss`). SOLD-path rows (`sold-profit`/`sold-loss`) are computed from real sell fills and were never affected.**

**⚠️ Two Polymarket data-API assumptions in this file are WRONG (verified 2026-07-29 against the live API):** (1) `redeemable=true` does **not** mean the position won — a sampled row came back `redeemable=true, curPrice=0, cashPnl` negative; (2) the `winning` query param is **ignored** when `redeemed=true` — `winning=true` and `winning=false` return byte-identical payloads. `backfillRealizedTrades` fetches both as if they were distinct win/loss buckets; dedup masks it today, but do not build anything new on either assumption.

**⚠️ Method note, learned the hard way 2026-07-29:** do not compare `realized_trades` against Polymarket's `/positions` redeemed bucket to detect fabricated wins. `realized_trades` records closed **round-trips** (buy, then sell — possibly long before resolution); `/positions` redeemed records positions **held to resolution**. They are different sets of trades and a sign difference between them proves nothing. A wallet's +853% `sold-profit` row (entry 10¢ → exit 95¢ on a market that later resolved NO) is legitimate and looks fabricated under that comparison. This mistake was made and published mid-session before being caught.

**⚠️ CORRECTION 2026-07-28 — taerv534 no longer qualifies; the 2026-07-21 hand-check of it is partially invalidated.** A separate bug (`_parseOutcomeSettlement` treating price extremity as proof of resolution, not gamma's own `closed` flag) had let 3,075 fabricated "redeemed" rows into `realized_trades` platform-wide — markets years from resolving (2027/2028 elections) counted as resolved trades at exactly ±100% ROI. 77 currently-qualifying wallets had at least one such row, including all three hand-checked here. After the corrective delete: **taerv534 dropped from n=10 to below the floor** (its qualifying n=10 apparently included a fabricated row — "highly selective, correctly so" was itself measured on partly-fake data). TB14 and MELOCOTON007 still qualify post-cleanup, so their specific hand-checked numbers stand, though TB14's n dropped by the 4 fake rows it had. Qualifying-wallet count moved 123 → 92 platform-wide. Ingestion bug fixed same day (`m.closed === true` now required); see SESSION_STATE.md 2026-07-28g/h for the full trace. Lesson: internal consistency (a wallet clearing the n≥10 bar) is still not truth — the discipline in this file's own "every number... has been wrong" section applies to the gate itself, not just the scores behind it.

**✅ LIVE — trader surface public as of 2026-07-23; homepage swapped to it 2026-07-26.** Provisional banners, `provisional`/`provisional_note` flags, and the per-card provisional badge are all removed; `/traders` is a real Express route serving the trader-first surface (`public/home-traders-preview.html`), linked prominently from site nav (`Traders`, first primary link). **⚠️ CORRECTION 2026-08-05: `/` no longer serves `home.html`.** `server.js:763` has routed `/` to `public/home-kings.html` ("King of the Castle" — connect-wallet hero + Top Trader card + category "Winning Trades" board) since 2026-07-26, per Marc's explicit call (see SESSION_STATE.md 2026-07-26) — the swap this file previously said was still pending already happened. `home.html` (the old market-grid page, with its own separate `.vcard` hero component) is unrouted/orphaned, kept on disk for reference/rollback only, reachable solely via a direct `/home.html` static path nobody links to. Do not edit `home.html` expecting it to affect the live homepage; edit `home-kings.html` instead. See SESSION_STATE.md 2026-08-05 for how this drift was found (flagged during a trader-card content-hierarchy pass that almost touched the wrong file).
...
**⚠️ Gate 3, landing-page clause — LIFTED 2026-08-24 by Marc, for visual work only.** He was shown the gate text verbatim and chose to override it; `public/home-kings.html` was reworked that session (hero board pulse, equity-curve sparklines, image hardening). **The publish-the-edge-number half of Gate 3 is untouched and still binding** — n=83 / 53.0% remains below bar and unpublished, and nothing derived from the grader was put on the page. Rule 2, the "do not build, do not polish" list and the voice charter were NOT lifted. Do not revert that visual work as a gate violation; see SESSION_STATE.md 2026-08-24. Whether the clause goes back on is Marc's call.

**Gate 4 — copy trading: SAFE (assisted, manual-confirm) tier only; auto-execution is a separate, later, audited-contract project, not scoped.** Formalized 2026-07-30 when assisted copy trading shipped (verified board → tap "Copy this" on an open position → existing `/market/:slug` trade widget prefilled → user's own wallet signs via the existing CLOB V2 flow). Non-custodial by construction: Hyperflex never holds funds, custodies, or holds delegated signing authority; every copy this build produces is a manual, per-trade, user-signed action. **Copy affordance only ever renders on a wallet that's actually on the verified durable board** (`durable_verified`, server-computed from `_computeRoiLeaderboard` — never inferred client-side) — a copy button on an unverified or fabricated record would be exactly the moat-destroying failure Gate 1 exists to prevent. The amount is always user-set, never auto-sized to the copied trader's position.
**⚠️ Found while building this, a MORE automated copy system already existed** — `public/copy-bot.js` (auto-loaded on every page via `nav.js`), `public/copy-trading.html`, and `/api/copy-bot/{subscribe,pending,stream,history,...}` in `server.js`. It let a user subscribe to a whale and get trades pushed via SSE, with an attempt at background execution using the user's own stored CLOB API keys + browser-wallet signature. Audited 2026-07-30: confirmed non-custodial (no raw private key stored, `HFXWallet.getSigner()` needs a live wallet signature per order) and confirmed **zero real trades had ever been auto-executed in production** (461 logged trade rows, 0 `filled`) — but the whale source was the old capital-deployed leaderboard with zero reference to the durable board, no daily spend cap existed, and it fired without per-trade review.

**Patched same day, per Marc's explicit call ("only the most consistent get to be auto traded, copy trading simply sends the signal... [real auto-trading] once we build the app"):**
1. Verified-board gate: `subscribe` force-downgrades any auto-execute request to notify-only unless the whale is durable-verified (checked again server-side in the trigger loop as defense-in-depth); UI surfaces the downgrade instead of failing silently.
2. Daily spend cap (`max_daily_spend`, default $200) + a 15-minute per-user cooldown between fires — neither existed before.
3. **The client-side auto-execute-in-background branch was removed entirely from `copy-bot.js`.** Every copy opportunity now shows a banner requiring an explicit "Copy →" tap — no code path fires a trade without that tap, for any subscription, verified or not. This is the concrete meaning of "copy trading simply sends the signal": bell notification + web push + email all fire on a whale event; nothing executes until the user acts. Real unattended auto-trading (for durable-verified traders specifically, once eligibility is that tight) is deferred to a future app-based build — not scoped, not started, matches this Gate's original framing.
...
**Primary surfaces:**
- **⚠️ CORRECTION 2026-08-05 — `/` is NOT explore.html.** `server.js:763` routes `/` to `public/home-kings.html` ("King of the Castle" — connect-wallet hero, Top Trader card, category "Winning Trades" board), live since 2026-07-26 per Marc's explicit call. `public/explore.html` is served at `/explore` directly (`server.js:34231`) — it does NOT redirect to `/`, it's the other way around: `/explore` is its own live route, separate from the homepage. The bullet below describing explore.html as "the front door" at `/` is stale; kept as-is beneath this correction rather than rewritten, since it's unclear whether explore.html is still meant to be a maintained surface at all — confirm with Marc before editing it further. See CLAUDE.md's Gate 1 section above for the current, accurate homepage description (home-kings.html) and SESSION_STATE.md 2026-08-05 for how this drift was found.
- **Landing / Explore** at `/` (file: `public/explore.html`) — the front door. Shows top edges, intelligence cards, live stats, and a **preview** of the FLEX Feed. NOT the full feed — users click through to `/feed` for the full experience. `/explore` redirects here (301). *(⚠️ See correction directly above — this bullet describes a routing that is no longer live.)*
...
|------|-------|-----------|
| `public/home-kings.html` | `/` (homepage) | **⚠️ Correction 2026-08-05 — this row was missing; `/` was wrongly listed as explore.html below.** "King of the Castle" homepage, live since 2026-07-26 (`server.js:763`, Marc's explicit call): connect-wallet hero, Top Trader card, "Winning Trades" category board (World/Politics/Macro/Sports). Cards render via `trader-card.js` (`HFXTraderCard.render`) for the Top Trader + hero-proof rotator, and a separate hand-rolled `winCardHtml()` for the category board — see the 2026-08-05 SESSION_STATE.md entries for the two-render-path gotcha. |
| `public/home.html` | unrouted (orphaned) | **Old market-grid homepage.** No live route serves it — only reachable via a direct `/home.html` static path. Kept on disk for reference/rollback, not maintained. Do not edit expecting it to affect the live site. |
| `public/explore.html` | `/explore` | **Landing-style page + feed preview.** Intelligence briefing, top edges, live stats, feed preview. ⚠️ NOT `/` — routed at `/explore` directly (`server.js:34231`), does not redirect from or to `/`. ⚠️ NOT the full feed — that's feed.html. Status as a maintained surface is unclear post-2026-07-26 homepage swap; confirm with Marc before investing more in it. |
...
15. **User-facing modals (trade composer, wallet connect, withdraw/deposit flow) use z-index ≥ 99998 to stay above third-party browser extension overlays.** Brave's annotation tool (`#NOTEPAD_controls`), Honey, Grammarly, Bitwarden, etc. inject `position:fixed` UI at high z-indices that can overlap our critical-flow modals if we sit at the same layer. Trade composer backdrop = 99998, sheet = 99999 (`public/market.html:9247-9249`). Don't fight every extension individually — keep modals in the 99998+ band and let extensions sit underneath. We don't actively hide third-party overlays; that's a slippery slope of fighting every extension that ships UI.
16. **Middleware helpers must be defined before the first route that uses them.** All Express middleware functions (`requireAuth`, `requireAdmin`, `requireAdminSecret`, and any future helpers) must be declared in a single block near the top of `server.js`, before any `app.get/post/put/delete` call. A `ReferenceError` thrown during synchronous route registration aborts the entire boot sequence — the `uncaughtException` handler never fires because it hasn't been registered yet. This is what caused the May 10 502 outage. When adding a new middleware function: (a) define it before line ~2000, (b) run `node --check server.js` before committing. The pre-push hook and CI `boot-check.yml` enforce the syntax check automatically.
17. **`node --check server.js` before every push.** The pre-push git hook (`.git/hooks/pre-push`) runs this automatically. CI (`boot-check.yml`) runs it on every push and also starts the server and hits `/health`. Never bypass with `--no-verify`. If the hook blocks you, fix the syntax error — don't skip the hook.

---

## ⚠️ Polymarket Safe-proxy derivation — DO NOT inline raw eth_call

**All server-side proxy derivation goes through `lib/polymarket-proxy.derivePolymarketProxy(eoa, rpcs)`. Never inline a raw `eth_call` to the Safe factory with a hardcoded selector.** The helper uses an ABI string (`function computeProxyAddress(address user) view returns (address)`) and lets ethers derive the selector at runtime. If the contract is migrated or the function renamed, ethers throws a clear `function not found` error at the first call — instead of the silent `execution reverted` that the previous broken-hardcoded-selector pattern produced for months while filling the 995-user "polymarket_proxy = NULL" bucket. Boot log emits `[boot] polymarket-proxy: factory=… selector=…`; smoke-verify with `curl /api/_smoke/polymarket-proxy?eoa=<X>&secret=$ADMIN_SECRET`. CI workflow `.github/workflows/no-stale-proxy-selectors.yml` blocks the legacy 4-byte selector from reappearing anywhere outside the incident-doc file (`lib/polymarket-proxy.js`).
