...
## 2026-08-25 (/markets rebuild — the page wasn't blank by design, the upstream query was broken; also removed a fabricated "edge" claim)

**Status: SHIPPED to `main`** — see the hash on origin (`git log origin/main --oneline -3`). Touched `server.js` (14 hunks, all inside `buildEdgeSignal` + `fetchFinanceMarkets`) and `public/finance.html` (full rebuild). `node --check server.js` passes; boot-check CI covers the rest.

**Root cause — one wrong word.** `fetchFinanceMarkets` asked gamma for `order=volume`. Gamma's `volume` column is a **string**, so the sort was lexicographic: a $99 market outranked a $99,947 one. Live measurement before the fix: `/api/finance/markets` returned **8 markets, all politics**, top row a $999 Lienchiang County Magistrate race at 0.1¢. The page looked blank because it was. Marc reported it as a styling problem; it was a data problem.

**Four more bugs found in the same function:**
1. `limit=500` — gamma hard-caps a page at **100** regardless. Now 10 pages via `offset`, fetched concurrently (behind the existing 10-min cache), each page failure-isolated so one bad page can't blank the board.
2. `sections.sports` existed but `PATTERNS` had no `sports` key, so it could never fill — and the frontend's `catOrder` omitted it to match. Both halves fixed.
3. `whale_count: 0` was **hardcoded at the emit site**, so `buildEdgeSignal`'s whale branch could never fire. Now enriched by `conditionId` off `_screenerCache` (buildAlphaList, per CLAUDE.md "Alpha Engine").
4. A 40–60% price-band cull threw away every market the book was genuinely split on — i.e. exactly the ones where a call is worth anything — so the "structural NO edge" copy would always have something extreme to talk about. Removed.

**Relevance fixes on top:** rank by **liveness** (`volume24hr`, falling back to lifetime/365) not lifetime volume — lifetime volume put "Will LeBron James win the 2028 US Presidential Election" (0¢, $54M lifetime, long dead) above the September FOMC decision. Multi-outcome legs capped at 2 per parent event (`events[0].id`) — one F1 championship event was otherwise eating a whole category with 11 legs at 0%. Volume floor $10 → $25k. `macro` patterns widened (was matching **1 market out of 400**; the set was almost entirely single-ticker equity phrasing and missed every standard macro release). Result: **8 junk markets → 71 real ones across all 5 categories.**

**⛔ Removed a fabricated statistic that was live on the page.** The banner asserted *"Markets priced under 10¢ resolve YES only 1.1% of the time"*, and `buildEdgeSignal` shipped *"under 0.5% of markets resolve YES"* / *"~1.1% historically resolve YES"* / *"~5% resolve YES"*. **Nothing in this codebase has ever measured any of those numbers.** They were also circular — at N¢ the market's implied probability IS N%, so restating it as a discovered edge is a tautology. Gate 3 bars publishing any grader-derived figure until n≥30 / 58% (currently 53.0%), and these weren't even grader-derived. Replaced with payout arithmetic (always true) plus **measured 24h/7d reprice** off gamma's own `oneDayPriceChange`/`oneWeekPriceChange` — the one signal on the page that isn't a restatement of the price. The frontend's `edgeStars()` five-star system (five stars for any price under 5¢) is deleted outright.

**Frontend:** `public/finance.html` rebuilt on home-kings.html's tokens/components so it reads as the same product — KPI row, three chart cards each with a TABLE twin (per the homepage's WCAG pattern), category pill rail, market rows with real signal chips, and a right-side detail sheet carrying a **real price-history sparkline** from `/api/market-history/:token_id` (verified live: 743 points, 41% → 68%). Old 3-column terminal layout dropped.

**Verified how:** cannot run `server.js` (CLAUDE.md rule 7), so a throwaway static server on **port 8899** served `public/finance.html` against a fixture generated by running the patched pipeline against live gamma, with `/api/img-proxy` and `/api/market-history` proxied to the real upstreams. Checked at 1280px and 375px. Found and fixed a real mobile bug this way: the movers-chart value labels were absolutely positioned outside their bar and landed on top of the question text at 375px — they now have their own grid column.

**Active blockers:**
- `whale-backed` KPI reads 0 in local verification because the fixture has no `_screenerCache`. **Unverified in production** — confirm it populates after deploy.
...
- Per CLAUDE.md rule 4, this page is the natural home for the unbuilt "markets by category for non-qualifying wallets" destination that `/connect` currently placeholders to `/traders`. Now that it has real category browsing, wiring `/connect` here is a small follow-up.

**Notes for next session:**
- Post-deploy: hit `/api/finance/markets` and confirm 5 non-empty sections and non-zero `whale_count` on at least some rows.
...
**What was built:** a rebuilt `/` (`public/home-kings.html`) plus two public endpoints — `GET /api/board-stats` (aggregate charts) and `GET /api/live-calls` (open positions held by verified-board traders, with a trade CTA). Product framing per Marc mid-session: the homepage's job is to push a visitor to actually place a bet, using a verified trader's record as the credibility. Gate 4 held throughout (verified-board wallets only, server-computed; CTA carries the site default $25 and never the trader's size; nothing auto-executes; score+n on every card).

**⛔ THE LESSON, and the reason this entry is long: every single real bug was invisible to the layer above it.** Fixtures passed while production was broken; the API returned clean 200s while the user-facing path was wrong; the page rendered perfectly while a number contradicted the tile above it. In order found:
1. **Fixtures green, prod broken.** A position at `curPrice` 0.9996 cleared a `< 1` check and shipped "Trade YES at 100¢" for +0.1%. Also multi-outcome markets shipping named outcomes ("ELENA RYBAKINA") as `side`, producing a CTA linking to a broken side param. Fixed: 3¢–95¢ band, YES/NO only, enforced on both sides of the wire.
2. **Valid ≠ worth promoting.** Prod was serving, under a "Trade this" button: a position the price had run +118% past, a position down −87%, a 30%-win-rate wallet, and an 809-day "countdown". Fixed: |move| ≤ 25% ("still gettable at roughly their price"), win rate ≥ 55%, horizon 24h–365d.
3. **API healthy, product wrong — the worst one.** Both the live-call CTA and the trader-card Trade button linked to a position's PARENT EVENT instead of the market the position is in. Nearly every Polymarket position is one leg of a multi-outcome event, so this was the common case. For a card quoting "Trade NO at 90¢": leg slug → "NO 89¢" (correct), event slug → "NO 96¢" (different market). Users were being sent to buy a market we never quoted. Caught only by fetching the CTA URL and diffing its `og:description` against the card's price. **`/api/polymarket/positions/:address` now also returns `market_slug` (the leg); `market_url` still points at the parent event, which is correct for "view on Polymarket".**
4. **Rendered page contradicted itself.** The capital donut summed to $137.9M under a tile reading "$163M capital graded" — `classifyCardCategory`'s `'other'` residual (15%, $25.1M) was excluded entirely. Now returned as `uncategorized` and folded into the donut's Other slice.
5. **Payload failed its own arithmetic.** `potential_roi_pct` was derived from the raw price but published beside a rounded one, so `(1-price)/price` on the published figures didn't reproduce the published return. Both now derive from the rounded price that ships.
6. **Load.** The homepage fired ~25 `/api/polymarket/positions/:address` calls on load (each ~5 upstream Polymarket calls, ≈125 per visitor, with duplicate wallets). Now IntersectionObserver-deferred + one shared in-flight promise per wallet.

**⚠️ FALSE DIAGNOSIS, CORRECTED — read this before trusting any browser-automation result in this repo.** Commit `327c348`'s message claims zero-area elements never fire IntersectionObserver and that a live Trade-button regression was verified. **Both claims are wrong.** Chrome *does* fire IO for a zero-width element (measured twice: `isIntersecting` true at `w=0`). The "25 pending, 0 filled, 0 requests" reading came from an automated browser whose document was `visibilityState: "hidden"` — **IO never fires in a hidden document**, which also froze the page's own card-entrance animation at opacity 0 (a tell that should have been caught immediately: if the pre-existing animation is also dead, suspect the tool, not the change). Re-verified against the live site in a *painting* headless Chrome with a tall viewport: **13 real Trade links render with correct slugs and prices.** The code change is retained (observing a non-degenerate box is more robust) but it fixed nothing. Corrected in-code at the callsite. **When debugging IO here, check `document.hidden` FIRST.**

**Second, deeper test pass (same day, after the above shipped).** Went past "does it respond 200" into consistency, interaction, responsive and accessibility. Found and fixed three more real defects:
- **12 of 23 text styles failed WCAG AA contrast** — measured 2.59:1–2.96:1 against a 4.5:1 requirement, including the risk disclosure, the listing notice, KPI labels, the payout line and chart axis ticks. **All twelve traced to one token, `--ink-faint` (#565d70).** Lifted to `#7d8599`, the minimum clearing 4.5:1 on the lightest surface (`--surface-2`). `--ink-muted` already passed (5.33–6.08) and was left alone so the two still read as distinct levels. **Do not darken `--ink-faint` again without re-measuring** — the offline suite now recomputes the ratio and fails on regression. Verified live: contrast failures 12 → 0.
- **Unnamed controls.** 25 Flex Score gauge SVGs had no accessible name (now `aria-hidden`; the score is already text beside them). Separately **three icon-only buttons in the shared `nav.js` had no name at all** — a screen reader announced three bare "button"s on *every page of the site*. Given aria-labels. Live: unlabeled buttons 3 → 0, unlabeled SVGs 31 → 6 (the six left are icons inside now-labeled buttons, which is correct).
- **The risk disclosure only existed after `/api/live-calls` resolved** — it was not in the served HTML. A disclosure on financial content must not depend on a fetch succeeding. Now static, with the endpoint's wording overwriting it on load. Keep the two wordings in sync.

**Verified clean in that pass (no action needed):** zero horizontal overflow at 320/375/768/1440; all four chart↔table twins populate with real data, captions and headers, and toggle back correctly; the score-info expander works; all five invalid address inputs are blocked with a clear message and a valid one navigates to `/connect`; **both failure modes degrade honestly** — on network failure every surface shows a real "Could not load…" message, on empty-but-valid payloads every surface shows its honest empty state, and neither leaks `NaN`, `undefined` or `[object Object]`.

**Testing built — now in the repo under `test/`, wired to npm scripts:**
- `test/homepage.test.js` — **121 assertions, offline, no network/DB/server.** Slices the page's OWN script block out of `home-kings.html` and eval's it against a DOM shim, so it tests shipped code and cannot drift from a copy. Covers nulls, empty states, XSS, extreme prices, countdown boundaries, Gate 4 sizing, YES/NO-only, donut summing to 100%, plus page-level a11y (static disclosure, aria-hidden gauges, one h1, and a recomputed `--ink-faint` contrast ratio). Mutation-tested: deliberately breaking the Gate 4 size rule fails it. `npm run test:homepage`
- `test/homepage-live.test.js` — **137 assertions against PRODUCTION.** Recomputes every published figure from the other published figures, then **walks each live call's CTA and diffs the destination page's price against the card's quote.** This is what caught the wrong-market bug; nothing at the API layer would have. `npm run test:homepage:live`
- `test/homepage-deep.test.js` — **183 assertions against PRODUCTION.** Cross-endpoint consistency (`board-stats` vs `kings` vs `category-leaderboard` — every promoted wallet must clear the same floor), arithmetic invariants (category trades + uncategorized == scored_trades, buckets + unscored == ranked_wallets, categories sorted by capital), input robustness (empty/`other`/SQL-ish category, malformed wallet address, junk query params — must 4xx, never 500), and caching/concurrency (a 12-request burst must all 200). `npm run test:homepage:deep`
- **All three green as of this entry: 121 / 137 / 183.**

⚠️ **`.github/workflows/homepage-tests.yml` is written and on disk but NOT committed** — the push token lacks GitHub's `workflow` scope and the remote rejects it (`refusing to allow a Personal Access Token to create or update workflow ... without 'workflow' scope`). Add it via the GitHub web UI or a PAT with that scope to get the offline suite gating pushes. The live and deep suites are deliberately manual — they hit production and would fail CI for reasons unrelated to a diff (a market resolving, Polymarket rate-limiting, a thin rail).

**Chart palette is validated, not chosen by eye.** Categorical hues `#3b82f6`/`#d97706`/`#db2777` are the largest set passing an all-pairs colour-vision-deficiency check against the `#12151b` surface (blue+purple fails at deutan ΔE 0.9; blue+cyan at normal-vision ΔE 12.3). Donut caps at top-3 + neutral Other. Green/red reserved for win/loss, never a category identity. Every chart has a table-view twin. Do not re-pick these by eye.

**Copy:** Marc called the original hero subline ("Not what you screenshotted") cheesy — replaced with a dry statement per the voice charter. CTA now reads "Trade YES at 41¢ **on Polymarket**", plus a plain-English explainer ("41¢ a share, pays $1 if YES resolves") because a first-time visitor is otherwise priced out by the jargon. Every market is guaranteed artwork: Polymarket's `icon` is the only image field and can be null or 404, so a deterministic tile (hue hashed from slug + first letter) fills in.

**Open / not done:**
- `ROI_MIN_N_FLOOR` board has grown to **455 ranked wallets** (CLAUDE.md still says 80 post-purge — stale, as that file warns). $163M capital graded, 25,302 scored trades, 62.8% board win rate. Read live from `/api/board-stats`.
- The two test suites live in the session scratchpad, not the repo. They should be committed under `test/` and wired into CI — `test-live.js` in particular is the only thing that would catch a repeat of bug 3.
- Still uncommitted from other sessions, untouched by me: `public/connect.html`, `public/feed.html`, `public/home-traders-preview.html`, `public/trader-card.css`, `public/trader-card.js`, `.DS_Store`. A local branch `pre-rebase-backup-2026-08-17` preserves two superseded local commits (`4dab1ab`, `5b61787`) whose work reached origin via PRs #225–#231 in refined form; they were deliberately not replayed during the rebase.
...
**Same-session follow-up, later 2026-08-06:** `dcccc90` (pushed, verified) — renamed 'Incentives' → 'Yield' and promoted it from the buried desktop 'More' dropdown into `primaryLinks` (Marc's ask). Verifying it surfaced a real, previously-invisible bug: nav.js's `@media(min-width:1024px)` "desktop scale-up" (24px links, 40px logo, full search pill) needed ~1900px of total width at the current `primaryLinks` count, so the search button and wallet pill were silently rendering off-canvas past the right edge at every normal laptop viewport (1280-1728px) — invisible unless you check `getBoundingClientRect()`, the accessibility tree alone won't show it since the elements are still technically present, just off-screen. Raised the breakpoint to `1900px` so the existing compact style (12px links, already comfortably fits ~1050px total even at 10 items, confirmed at 1000px viewport) covers the whole realistic desktop range; the scale-up now only engages on genuinely wide monitors. Verified no overflow at 1280/1440px, scale-up still engages correctly at 2200px. **Worth a pass elsewhere:** this class of bug (element present in DOM/a11y-tree but positioned off-viewport) won't show up in a routine `read_page`/`get_page_text` check — if a future session is debugging "user says X isn't there but I can't find anything wrong," check bounding-box positions against viewport width, not just presence.
...
- **🚨 MEASURED, AND IT IS BAD. TheQuietRisk (ranked #5, promoted): 18 of 110 markets affected, $202,044 of realized P&L missing from its record.** Two examples from the live trace:
  - *"US forces enter Iran by April 30?"* — 150 buys ($508,188), one sell ($798,377) = **+$290,189 realized win, entirely absent.** `segments_written: []`, `stored_rows: []`. Cause: `still_open_shares: 0.0015` — 800,779 shares closed, fifteen ten-thousandths of a share of float dust left, so the exact-zero flat check never fired and the whole segment was discarded.
  - *"Will the Iranian regime fall by June 30?"* — **−$225,899 realized loss, entirely absent** (3,953 shares genuinely still open out of 2.2M closed).
- **Two distinct sub-cases, different risk profiles — stage the fix accordingly.** (a) **Rounding dust** (<1 share remaining): the position IS closed and we dropped it on a floating-point technicality. Safely fixed with a tolerance on the flat check; no key-format hazard, because such a segment won't meaningfully gain further sells. (b) **Genuine partial exit**: needs the key redesign below.
- **⚠️ This is a Gate 1 exposure, not just a data bug:** we are promoting traders on records that omit six-figure realized losses. The board's honesty is the moat.
- **Shipped read-only sizing:** `POST /api/admin/measure-dropped-pnl?confirm=1` + `/status` — background scan across every ranked wallet, reports `total_dropped_pnl_usd` and `total_dust_pnl_usd` and the worst 15. Run before touching scoring.
- **✅ FULL FIX SHIPPED (Marc's call: fix both sub-cases together, board stays up with disclosure).** Live scan read 146 of the first 150 ranked wallets affected — **97%**, so this was the norm, not an edge case.
  1. **Flat is now a tolerance, not an exact zero** — `openShares <= max(1, closed * 1e-6)`. Recovers the dust case (0.0015 shares blocking a $290K win) while correctly leaving a genuine partial exit (3,953 open vs 2.2M closed) open.
  2. **Trailing open segments are now written.** Realized P&L on the sold portion is a real trade; the rest is simply an open position.
  3. **New key `pm-act3:<user>:<cond>:<outcome>:<segIndex>`.** Positional index is stable because past events are immutable and activity only appends, so a trailing segment keeps ONE row and refreshes in place via `ON CONFLICT DO UPDATE` — keying on `lastCloseAt` (the pm-act2 scheme) would have minted a new row per backfill and double-counted every earlier partial.
  4. **Per-group reconciliation IS the migration.** Backfill deletes that group's `pm-act:`/`pm-act2:` rows plus any stale `pm-act3:` row in the same statement that writes the new set — without this, new rows would have landed alongside old ones and double-counted every trade platform-wide.
  5. **Truncation guard.** A wallet whose /activity feed hits the 10k cap has incomplete history, so ALL sold-path writes are skipped: deleting against a non-authoritative set destroys good rows, writing without deleting double-counts. Such wallets keep existing rows, stale but never doubled. **The resync's blanket pre-delete was REMOVED for this reason** — pre-delete + skipped backfill would have wiped the record entirely.
  6. Resync now selects `pm-act:` OR `pm-act2:` so the whole platform migrates automatically.
- **Disclosure live:** site-wide notice in `nav.js` (single `HF_SCORE_NOTICE` flag, one block — **remove it when the rebuild completes**) plus a correction card on `/methodology` naming the real numbers. `nav.js` cache version bumped 25 → 26 across all 41 pages or the notice would not have appeared.
- **⚠️ FOLLOW-UP, found watching the first live batch:** tick 1 processed 50 wallets, `n_gained: 2127`, `old_rows_deleted: 739` — the fix is recovering ~42 trades per wallet. But wallet `002d9333` came back `old_format_rows_retired: 0, n_gained: 0, changed: false` — the truncation guard had skipped all its writes. **Because selection is deterministic (`ORDER BY … user_id`), a wallet that never migrates is re-picked every tick forever and pins the front of the queue, so the backlog would have stalled.** Fixed three ways: activity cap raised 10k → 40k (largest wallet observed is 5,501 events, so this covers reality with headroom); `backfillRealizedTrades` now returns `truncated` so a refusal-to-write is distinguishable from "nothing to do"; and `_resyncNoop` skips any wallet that changes nothing twice (flagged/targeted wallets exempt), surfaced as `skipped_no_op` in status. **Lesson: a deterministic queue plus a skip-on-failure path is a livelock waiting to happen — anything that can decline to make progress must also be removable from selection.**
- **NEXT:** watch the resync drain (`/api/admin/resync-sold-trades/status` — `wallets_remaining` now counts both old formats), then re-run `measure-dropped-pnl` and expect the dropped total to collapse. Then remove the notice and re-verify promoted wallets.
- **Superseded note (kept for history) — the original framing of what was NOT done:** Writing the trailing segment is not a one-line change: the key is `pm-act2:user:cond:outcome:<lastCloseAt>`, and a trailing segment's `lastCloseAt` advances on every new partial sell, so naively emitting it creates a NEW row each backfill while the old one survives → double-counted P&L. A correct fix needs either a stable key component (`firstOpenAt`) plus a new prefix + full resync, or per-group reconciliation (delete rows for that group not in the freshly-computed key set). Measure with segment-trace first.
