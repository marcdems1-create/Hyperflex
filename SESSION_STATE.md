...
**Testing built (reusable, in the session scratchpad — worth relocating into the repo):**
- `test-home.js` — 112 assertions against the page's REAL render functions (sliced + eval'd from `home-kings.html`, so it tests shipped code, not a copy). Covers nulls, empty states, XSS, extreme prices, countdown boundaries, Gate 4 sizing, donut summing to 100%. Mutation-tested: deliberately breaking the Gate 4 size rule fails the suite.
- `test-live.js` — 137 assertions against PRODUCTION. Recomputes every published figure from the other published figures, then **walks each live call's CTA and diffs the destination page's price against the card's quote.** This is the suite that caught bug 3; nothing at the API layer would have. Currently 137/137.
