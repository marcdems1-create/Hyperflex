...
- `999f1ee` — this SESSION_STATE.md entry.
- `1251456` — **the 434a814 pass missed a second render path.** Marc's eyeball check on the deployed page: Top Trader card + hero-proof rotator correctly showed the Flex Score hero, but the "Winning Trades" category board (World/Politics/Macro/Sports) still showed the old raw-ROI-in-a-corner badge. Root cause: that board renders through a separate hand-rolled `winCardHtml()` in `home-kings.html`/`feed.html` (backed by `GET /api/feed/category-wins`), not `trader-card.js`'s `HFXTraderCard.render()` — a second render path that didn't inherit the first fix. Added `flex_score`/`win_rate_pct`/`raw_weighted_roi_pct` to that endpoint's response (server.js, sourced from the already-built `_buildTraderCards` card), and gave both `winCardHtml()` copies the same gold `.wcard-flexbadge` hero + `.wcard-substats` row treatment, matching `.tcard-flexhero`/`.tcard-stats` visually. The card's own win evidence (multiplier/entry/question) stayed untouched below the score. Verified end-to-end with a Playwright render against the real page + real CSS (mocked API responses) — screenshot confirmed the Top Trader card and category-board cards now visually match. Also fixed the CLAUDE.md file-map/Primary-surfaces staleness this session found (see below) as part of the same commit.
- CLAUDE.md fixed in place (no separate hash — see `1251456`): the ⛔ Gate-1 paragraph, the "Primary surfaces" bullet list, and the File Map table all previously claimed `/` serves `home.html` or `explore.html`. Corrected to state `home-kings.html` serves `/` (live since 2026-07-26, `server.js:763`), `home.html` is unrouted/orphaned, and `explore.html` is served at `/explore` (not `/`, does not redirect either direction).
- `9b18a16` — Marc reported the connected pill still routing to `/feed` on at least one of desktop/mobile. Investigated thoroughly: `git show origin/main:public/nav.js` is **byte-identical** to this branch's nav.js — both the desktop `#navWalletBtn` click handler and `mobileWalletLink.href` (set in `setConnected()`) already read `/trader/' + encodeURIComponent(current-or-addr)`, confirmed via `git show`/grep on both refs. No other file in `public/` references `navWalletBtn` or the wallet-pill click path (checked `wallet.js`, `fund-proxy.js`, `copy-bot.js`, `deposit.js`) — nav.js is the only implementation. Re-verified live behavior with a Playwright render of the actual `home-kings.html`/`feed.html` files (mocked wallet + API responses): desktop click navigates to `/trader/0x4349...fc6d`, mobile link's `href` attribute is the same. **Could not reproduce the bug in source on either ref.** Bumped `nav.js?v=25` → `?v=26` across all 41 pages that load it as a defensive fix in case a CDN/edge cache was serving a stale pre-fix nav.js under the old versioned URL (origin sets `Cache-Control: no-store` on `.js`, but that doesn't guarantee every layer between origin and browser honors it) — cheap, harmless if that wasn't the cause, real fix if it was. Flagged to Marc: if it's still showing `/feed` after a hard refresh post-deploy, the bug is somewhere this investigation didn't reach (worth screen-recording the exact click, since the code genuinely doesn't contain a `/feed` branch anywhere in the wallet-connect path).

**Found, not fixed (informational):**
...
**Open questions / unverified:**
- The nav fix (`2cd9ac7`) has not been confirmed against the actual live site — this environment's outbound proxy 403s all requests to `hyperflex.network`. Verified via a local static-file Playwright harness against the real modified file instead. The card-hierarchy fixes WERE confirmed against Marc's live eyeball feedback (the `1251456` fix exists because he caught the category-board split after deploy) — that's the more reliable verification loop going forward: ship, ask Marc to look, fix what he reports, don't assume "renders correctly in a local harness" means "matches production."
...
**Notes for next session:**
- If Marc's next "nudge" pass on the card hierarchy wants the Flex Score hero bigger/smaller/different color, there are now TWO places to change in lockstep, not one: `trader-card.css`'s `.tcard-flexhero`/`.tcard-flexscore`/`.tcard-stats` (Top Trader card, hero-proof rotator, `/traders`) AND `home-kings.html` + `feed.html`'s inline `.wcard-flexbadge`/`.wcard-substats` CSS (category "Winning Trades" board). They're deliberately separate implementations (different card purpose — trader summary vs. win evidence), so don't assume editing one covers the other. If a THIRD render path for trader score+n turns up looking inconsistent again, it's worth asking whether unifying all of them onto `HFXTraderCard.render()` (reshaping `/api/feed/category-wins`'s response to match trader-card.js's card contract, using its existing `evidence` field for the win details) is worth the refactor — flagged, not done, this session kept the two paths separate and patched both by hand.
- `home.html` vs `home-kings.html` confusion is now fixed directly in CLAUDE.md (Gate 1 section, Primary Surfaces list, File Map table) — don't re-flag it, it's resolved.
