...
### fix (same day, found by checking the DEPLOYED payload rather than the local fixture)
- **Whale enrichment was a silent no-op in production: 0 of 71 rows.** The map keyed on `sm.conditionId`, but `_screenerCache.data` rows have no such property — `buildAlphaList` builds the id as `m.conditionId || m.slug || m.id` and emits it under the name **`market_id`** (server.js ~43538). Every row hit `continue`. The endpoint returned clean 200s and the page rendered perfectly the whole time; only diffing the live payload showed it. Now indexes `market_id`, `conditionId` and `slug`, and looks up by conditionId then slug. **This is the CLAUDE.md lesson again — internal consistency is not truth. The local fixture had no `_screenerCache` at all, so the bug was structurally invisible to local verification.**
- **`buildEdgeSignal` printed the literal string "pays null× if it lands".** `Math.round(0.004 * 100)` is `0`, so `100 / pct` divided by zero and `.toFixed(1)` on `null`… the guard returned `null`, which template-literal'd straight into user-facing copy. It also claimed "market implies 0% odds" for a market implying 0.4%. Now uses the exact price for the multiplier and the shared `_fmtChanceDisplay` for display (`0.4¢`, not `0%`), and drops the payoff clause entirely when the price is genuinely 0.

### fix(server.js `buildAlphaList`): the same string sort was in the alpha engine, and it was worse there
- Chasing why `/markets` still showed 0 whale-backed markets after the key fix led to the source. **`_screenerCache` held 9 rows with zero whales matched platform-wide** — `/api/screener` confirmed it live (`rows: 9`, `max whale_count: 0`). The whale enrichment was pointed at an empty well.
- Root cause identical: `markets/keyset?limit=500&order=volume&ascending=false`. Measured live, that returns markets ordered `100, 100, 100, 100, 1000, 100, 99948…` — a $100 market above a $99,948 one. The `$10k 24h volume` floor then culled that near-random 100 down to **9**.
- The in-code comment defending it was wrong twice: (a) *"we sort client-side by 24h volume anyway, so the server-side sort just needs to be a valid parameter"* — the server sort decides **which markets are ever fetched**; re-sorting 100 near-random rows cannot recover the ones never returned. (b) *"markets/keyset only accepts 'volume' (not 'volume24hr')"* — verified live, keyset sorts correctly on **both** `volumeNum` and `volume24hr`. Only `volume` is broken.
- Fixed to `order=volumeNum` (densely populated; gamma stamps `volume24hr` on only ~half of rows). The existing client-side `_vol` sort still applies the 24h preference within the window, so downstream logic is unchanged.
- **This changes the input universe for every surface reading `buildAlphaList`** — `/alpha-live`, `/api/screener`, `/api/signals`, `/api/alpha/top`, edge-pick logging, brag moments. All of them were running on ~100 near-random low-volume markets. Correcting, but broad — worth a look at `/alpha-live` after deploy.
