...
### fix: V2 BUY wrap — pick USDC.e OR native USDC depending on what the proxy actually holds
- **File:** `public/creator-dashboard.html` → `dashWrapUsdcToPmct()`, new `DASH_USDC_NATIVE_ADDRESS` constant, new generic `dashIsErc20ApprovedForSpender` + `dashApproveErc20ForSpender` helpers
- **Symptom after PR #42:** tester retried BUY, saw 5 MetaMask popups, the wrap tx failed anyway. Railway logs showed the `/safe-submit` request body still targeting `0x93070a847e…` (CollateralOnramp) with body length `748` consistent with `wrap()`. The approve call we added in PR #42 wasn't visible in the log — suggesting the inline check found the allowance cached (previous approvals had landed), skipped the approve, dispatched the wrap, and MetaMask STILL sim-reverted.
- **Root cause:** our wrap hardcoded USDC.e as the asset (`wrap(USDC_E, proxy, amount)`). Polymarket's onboarding deposit flow can land funds as **native USDC** (`0x3c499c…`) instead of USDC.e (`0x2791Bca1…`) depending on the bridge / widget used. `CollateralOnramp.wrap(asset, to, amount)` does `IERC20(asset).transferFrom(msg.sender, self, amount)` — if `asset` is USDC.e and the proxy holds zero USDC.e (because the deposit was native USDC), the transferFrom reverts at simulation time → MetaMask "likely to fail" → user cancels → no pUSD ever mints. The tester's polymarket.com deposit showed `Balance: $2.00` in the UI's aggregate read, but the $2 was native USDC, not USDC.e. We kept trying to wrap from an empty USDC.e balance.
- **Fix:** `dashWrapUsdcToPmct` now reads BOTH balances on-chain (cache-bypassed, via `balanceOf` against each USDC contract) and picks whichever token has enough for the wrap. Prefers USDC.e when both are sufficient. Throws a clean "insufficient USDC" error with both balances dumped if neither covers the wrap amount.
- **Approval cache fix:** the previous `dashIsUsdceApprovedForSpender` cache key was `hfx_usdce_ok_<owner>_<spender>` — USDC.e-specific but NOT keyed on token. If we approved USDC.e to the Onramp in a prior attempt, the cache key said "approved", and switching to native USDC would read the same cache → falsely skip the new approval. New helpers (`dashIsErc20ApprovedForSpender` + `dashApproveErc20ForSpender`) key on `(tokenAddr, owner, spender)` so USDC.e and native USDC have independent cache entries. The wrap function uses these now; the older USDC.e-specific helpers (added in PR #41) stay for the V2 allowance pre-flight's USDC.e-to-Onramp entry, which doesn't suffer the aliasing issue.
- **Instrumentation:** `[dash-pmct] proxy USDC balances | USDC.e=X native=Y needed=Z` logs on every wrap so Railway shows exactly what the proxy holds. If future users hit "insufficient USDC", the log lets us tell whether the issue is "deposit didn't land" vs "deposit landed as wrong variant" vs "amount math is off".
- **Don't break:** both USDC variants are now handled. Do NOT add a third approval path for the other token "for safety" — users without that token simply won't go through that branch. `DASH_USDC_NATIVE_ADDRESS` is a new sibling to `DASH_USDC_E_ADDRESS`; if you ever rename the constants, keep the native-vs-bridged semantic clear (native is the newer Circle deployment, USDC.e is the older bridged). Do not consolidate `dashIsErc20ApprovedForSpender` with `dashIsPmctApprovedForSpender` — pUSD's 1M-atomic threshold vs ERC-20 generic is the same today but coupling them forever is a trap if Polymarket ever changes the pUSD decimals.
