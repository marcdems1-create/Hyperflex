...
// Railway's public TCP proxy (*.proxy.rlwy.net) is what times out when the
// plugin's DATABASE_URL is the public one. Same user/password/db on the
// private hostname usually still works. Do not log the returned URL.
function withPgHostPort(url, host, port) {
  const u = new URL(String(url).replace(/^postgresql:/i, 'http:'));
  u.hostname = host;
  u.port = String(port);
...
function deriveAlternatePgUrls(url) {
  const out = [];
  if (!url) return out;
  try {
    const u = new URL(String(url).replace(/^postgresql:/i, 'http:'));
    const host = u.hostname || '';
    if (/\.rlwy\.net$/i.test(host) || /\.proxy\.rlwy\.net$/i.test(host)) {
      out.push({ name: 'derived-private', url: withPgHostPort(url, 'postgres.railway.internal', 5432) });
    }
  } catch (_) { /* ignore unparseable */ }
  return out;
}

function collectPgCandidates(env) {
  env = env || process.env;
  const seen = new Set();
  const list = [];
  const add = (name, url) => {
  ...
  add('DATABASE_PRIVATE_URL', env.DATABASE_PRIVATE_URL);
  add('DATABASE_PUBLIC_URL', env.DATABASE_PUBLIC_URL);
  if (env.DATABASE_URL) {
    for (const d of deriveAlternatePgUrls(env.DATABASE_URL)) add(d.name, d.url);
  }
  return list;
}
...
function makePoolConfig(connectionString, extra = {}) {
  ...
  return {
    connectionString,
    ssl: extra.ssl === false ? false : { rejectUnauthorized: false },
    max,
