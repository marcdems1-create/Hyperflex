...
//   what-will-<speaker>-say-during-<month>-press-conference
//   what-will-<speaker>-say-during-<month>-<year>-press-conference
// Bypasses the keyword search entirely. We hit getEventBySlug for
// each speculative slug and accept any that resolve. Cheap: one
// gamma call per slug, ~96 slugs total (11 speakers × 3 months ×
// 2 year-suffix variants × 1.5 pattern variants), and gamma caches
// slug lookups for 5min upstream so warm hits are free.
const MONTH_NAMES = [
...
function _slugProbeCandidates(speakers, opts = {}) {
  ...
  for (const speaker of speakers) {
    ...
    for (let i = 0; i <= monthsAhead; i++) {
      ...
      candidates.push(`what-will-${sp}-say-during-${month}-press-conference`);
      candidates.push(`what-will-${sp}-say-during-${month}-${year}-press-conference`);
      candidates.push(`what-will-${sp}-say-during-${month}-${year}-fomc`);
    }
  }
  return candidates;
}
...
async function _selectFresh({ speakers, perSpeakerLimit, perPatternLimit, returnFunnel }) {
  ...
  for (const hit of probe.hits) {
    ...
    if (group.markets.length) eventGroups.set(eventSlug, group);
  }
