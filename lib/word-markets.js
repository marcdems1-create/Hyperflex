...
async function _sweepKeyword(term, perTermLimit, tag) {
  const url = `${GAMMA}/markets/keyset?closed=false&limit=${perTermLimit}&order=volume&ascending=false&search=${encodeURIComponent(term)}`;
...
async function _collectCandidates({ speakers, perSpeakerLimit, perPatternLimit }) {
  ...
  function _processSweep(kind, term, results) {
    ...
    const entry = bucket[term] = { fetched: results.length, after_pattern_filter: 0, after_speaker_attribution: 0, samples: [] };
