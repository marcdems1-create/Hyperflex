...
function _normalizeMarketCandidate(m, attributedSpeaker, matchedPatterns) {
...
async function getHeatmapCandidates(opts = {}) {
  ...
  // PRIMARY -- slug-probe via getUpcomingWordMarketEvents.
  //
  // Production diag (2026-05-14): the gamma `?search=will+say` /
  // `?search=mention` / etc. queries we used in PR #153 return mostly
  // unrelated junk ("Rihanna Album", "Jesus Christ return"). gamma's
  // full-text search is loose-token, not language-aware. The
  // slug-probe layer in getUpcomingWordMarketEvents()
  // (`what-will-<speaker>-say-during-<month>-<suffix>`) is the actual
  // working sourcing pipeline and finds the canonical 3 events x 52
  // sub-markets ($6.94M total).
  //
  // We call getUpcomingWordMarketEvents() and FLATTEN its event groups
  // into per-market candidates. Each sub-market becomes its own
  // heat-map tile, attributed to the parent group's speaker.
  const eventGroups = await getUpcomingWordMarketEvents({
    limit:           50,
    perSpeakerLimit: perTermLimit,
    perPatternLimit: perTermLimit,
  });

  const byCid = new Map();
  const funnel = {
    slug_probe_primary: { event_groups: 0, sub_markets: 0, kept: 0 },
  ...
  if (Array.isArray(eventGroups)) {
    funnel.slug_probe_primary.event_groups = eventGroups.length;
    for (const group of eventGroups) {
      const speaker = group.speaker || null;
      const speakerSlug = speaker ? String(speaker).toLowerCase().replace(/[^a-z0-9-]+/g, '') : null;
      // Per-event probe log per Marc's spec. Surfaces in vs out per group
      // so we can localize which event is dropping sub-markets if any do.
      const _flattenInBefore = funnel.slug_probe_primary.sub_markets;
      const _keptBefore      = funnel.slug_probe_primary.kept;
      const _groupMarkets    = Array.isArray(group.markets) ? group.markets : [];
      for (const m of _groupMarkets) {
        const cid = m.condition_id;
        if (!cid) continue;
        funnel.slug_probe_primary.sub_markets++;
        // Don't filter on m.closed here. Polymarket marks "Will Powell say X"
        // sub-markets as closed:true in TWO distinct states (per word-markets
        // PR #139): closed-because-resolved (event already happened) AND
        // closed-because-pending-activation (parent event listed but children
        // not yet trading -- typical 2-4 weeks before FOMC). Filtering on
        // closed=true here drops all 52 sub-markets across the 3 Powell
        // press-conference events (Railway log 2026-05-14: events=3
        // submarkets=52 kept=0). The downstream volume filter handles the
        // resolved-vs-pending split correctly: resolved markets have
        // volume_24h ~= 0 and fail the gate; pending-activation markets
        // with real lifetime volume pass on volume_total >= MIN_VOLUME_TOTAL.
        ...
        byCid.set(cid, {
          ...
          closed:           m.closed === true,
          active:           m.active !== false,
          matched_patterns: ['slug_probe_primary'],
        });
        funnel.slug_probe_primary.kept++;
      }
      // Per-event flatten probe -- one line per Polymarket event group.
      // `in` is the count of sub-markets carrying a condition_id (raw input
      // to the flatten); `out` is the count actually written to byCid.
      // Mismatch => something is dropping rows mid-flatten (dedupe, missing
      // condition_id on raw row, etc.). Identical => row makes it to volume
      // filter unchanged. Slug, kept-delta, and in-delta scoped to THIS group.
      const _flattenIn  = funnel.slug_probe_primary.sub_markets - _flattenInBefore;
      const _flattenOut = funnel.slug_probe_primary.kept - _keptBefore;
