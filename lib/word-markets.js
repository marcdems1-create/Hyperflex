...
async function getHeatmapCandidates(opts = {}) {
  ...
  // Per-pattern parallel sweeps. Tag each result row with which pattern
  // term-set it came from for funnel + matched_patterns attribution.
  ...
  // condition_id -> candidate. First-write wins on the row; matched_patterns
  // accumulates across patterns so a market that hits multiple sweeps is
  ...
  function _processSweep(label, sweepResults, requireSpeaker, allowFomcBody) {
    for (const { term, results } of sweepResults) {
      ...
      for (const m of results) {
        ...
        if (byCid.has(cid)) {
          // Already collected -- just tag the additional pattern.
        ...
        // Skip explicitly closed markets -- heat-map is live mentions only.
