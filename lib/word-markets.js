...
async function _runFutureEventDiscovery(opts = {}) {
  ...
  try {
    ...
    // surfaces high-volume future events first; helper translates camelCase
    // opts → snake_case URL params (gamma silently ignores camelCase).
    events = await polymarket.fetchPolymarketEvents({
      active:      true,
      closed:      false,
      endDateMin:  new Date(nowMs).toISOString(),
      endDateMax:  new Date(horizonMs).toISOString(),
      order:       'volume',
      ascending:   false,
      limit,
    });
