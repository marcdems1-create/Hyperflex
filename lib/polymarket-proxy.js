...
// hardcoded a legacy 4-byte selector — a fictional value that doesn't
// match any function on the live Safe factory. Every server-side proxy
// derivation reverted as a result; the client path at
// creator-dashboard.html:16527 works because it uses ethers' Contract
// ABI encoding to derive the correct selector at runtime.
//
// What was wrong:
//   The broken hardcoded selector had three different comments in
...
//   Computed selectors for the actual function names (verified with
//   `ethers.id(...).slice(0,10)`):
//     0xc4552791  proxies(address)
//     0xd600539a  computeProxyAddress(address)  ← the real one
//     0x1a65b51a  proxyFor(address)
//   None of those matched the hardcoded value. It was invented or
//   mis-typed once and then propagated by copy-paste across six call
//   sites over months.
//
...
//      the build if the legacy 4-byte selector reappears anywhere in
//      the repo outside of this file's docstring.
