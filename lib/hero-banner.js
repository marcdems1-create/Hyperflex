...
// 365-day horizon (was 90 in the original locked spec). The 90 was the
// right cap when the spec was "Warsh FOMC + near-term FOMCs only" — that
// context changed when narrative tracks landed (midterms 178d, BTC
// year-end ~235d, long-horizon AI Race resolutions). The selection gate
// is the (days_until_end * DAYS_PENALTY) deduction in _scoreEvent, NOT
// this horizon: a 365d event eats 36,500 penalty points just for being
// far, so the algo naturally surfaces near-term high-volume events
// without needing an arbitrary ceiling. The cap is now a hard "way too
// far" guard, not selection logic. Future-me: don't undo this — read
// the 2026-05-10 strategy thread before reducing.
const MAX_DAYS        = Number(process.env.HERO_BANNER_MAX_DAYS)     || 365;
