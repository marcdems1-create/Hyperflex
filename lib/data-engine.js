 ...
 *
 * Normalizes data from Polymarket + sports books into a single schema.
 ...
 *
 * Kalshi was dropped April 30, 2026 in the Polymarket-native pivot. The
 * fetch path + cross-match branches were ripped 2026-05-08; user-table
 * legacy columns + UI surfaces are deferred to a follow-up rip (separate
 * PR — broader blast radius than the data-engine path).
 *
 ...
 * @property {string} source          - "polymarket" | "sportsbook"
...
async function fetchPolymarketMarkets() {
  ...
  // /markets/keyset returns { data: [...], next_cursor } envelope as of the
  // May 2026 rollout, but legacy /markets used to return a bare array.
  // Accept both shapes — same logic as server.js _gammaUnwrap.
  const raw = Array.isArray(body) ? body
    : (body && Array.isArray(body.data)) ? body.data
    : (body && Array.isArray(body.markets)) ? body.markets
    : [];
...
async function refreshAll() {
  ...
  const [polyResult, sportsResult] = await Promise.allSettled([
  ...
  const allMarkets = [...polyMarkets, ...sportsMarkets];
  const byId = new Map();
  ...
  console.log(`[data-engine] Refresh: ${polyMarkets.length} poly + ${sportsMarkets.length} sports = ${allMarkets.length} markets, ${crossRefs.length} cross-refs (${elapsed}ms)${errors.length ? ' ERRORS: ' + errors.join('; ') : ''}`);
