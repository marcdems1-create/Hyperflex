...
// recompute-flex).
const CODE_VERSION = '2026-05-15-filter-resolved-tiles';
...
// Derive tile state from yes_price + end_date. Server-side derivation
// so the UI can render a PENDING/RESOLVED chip without re-implementing
// the rule. Live tiles render no badge.
//   - settled price + end_date in past   -> 'resolved' (event happened)
//   - settled price + end_date in future -> 'pending'  (pre-activation placeholder)
//   - non-settled price                  -> 'live'     (real trading underway)
// Markets with null yes_price treated as 'live' (price unknown != settled).
function _deriveState(c) {
  const p = c.yes_price;
  const settled = (p === 0 || p === 1);
  if (!settled) return 'live';
  const t = c.end_date ? Date.parse(c.end_date) : NaN;
  if (Number.isFinite(t) && t < Date.now()) return 'resolved';
  return 'pending';
}
...
function _composeResponse(filtered, funnel, totalCandidatesBeforeFilter, droppedSettled, droppedResolved) {
...
async function buildHeatmapResponse(opts = {}) {
  ...
  //   2. Resolved-state drop (this PR, 2026-05-15): Option C kept tiles
  //      with settled price + any volume signal, but those still split
  //      between "pending activation" (end_date future) and "resolved"
  //      (end_date past). Marc 2026-05-15: 52 of 53 surviving tiles
  //      rendered with RESOLVED badge -- "live mention markets" heat-map
  //      shouldn't be a graveyard. Drop resolved here; pending stays.
  //   3. Volume gate
  ...
  const filtered = notResolved.filter(_passesVolumeFilter);

  const response = _composeResponse(filtered, funnel, candidatesBeforeFilter, droppedSettled, droppedResolved);
...
module.exports = {
  ...
  MIN_VOLUME_24H,
  MIN_VOLUME_TOTAL,
  _internals: { _passesVolumeFilter, _passesLivenessFilter, _bucketIntoTiers, _composeResponse, _sortDesc, _stripEventBoilerplate, _deriveState },
};
