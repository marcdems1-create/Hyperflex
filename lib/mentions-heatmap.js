...
function _heatmapScore(c) {
  ...
  const controversy = 1 - Math.abs(p - 0.5) * 2;
  return (vol24 * 4 + volT * 0.5) * (controversy + 0.1);
}
...
function _composeResponse(filtered, funnel, totalCandidatesBeforeFilter, droppedSettled, droppedResolved, droppedSettledBinaries) {
  ...
  const capped = _diversityCap(filtered);
  const tiers = _bucketIntoTiers(capped);
  ...
  const total_volume_24h = capped.reduce((s, c) => s + (c.volume_24h || 0), 0);
  const total_volume     = capped.reduce((s, c) => s + (c.volume_total || 0), 0);

  const now = Date.now();
  return {
    total_markets:        capped.length,
