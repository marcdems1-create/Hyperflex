...
// recompute-flex).
const CODE_VERSION = '2026-05-15-editorial-exclusion-v1';
...
// Editorial scope filter (PR #170, 2026-05-15). /mentions is mention-betting
// on political/macro figures — not pop-culture meme markets. The future-event
// discovery pass surfaces high-volume "Trump out before GTA VI?" / "Will
// Jesus Christ return before GTA VI?" tiles because their slugs do contain
// the speaker watchlist word ("trump"). Single regex, tested against
// event/market slug + question text. Adding new exclusions = extend regex.
// If editorial scope ever needs a real allowlist, refactor then — for one
// off-thesis tile, one regex is the right hammer.
...
function _toResponseMarket(c, tier) {
  return {
    ...
    end_date:             c.end_date,
    endDate:              c.end_date,   // PR #170 alias for verification curl camelCase access
...
function _composeResponse(filtered, funnel, totalCandidatesBeforeFilter, droppedSettled, droppedResolved, droppedSettledBinaries, droppedEditorial) {
  ...
  return {
    ...
    sourcing: {
      ...
      patterns_shipped:   ['future_event_primary', 'word_betting_secondary', 'statement_betting_secondary', 'speaker_action_secondary', 'event_decision_secondary'],
      patterns_deferred:  ['tag_based'],  // v1.1 -- gamma /tags shape unverified
      primary_source:     'future_event_discovery',
      secondary_note:     'keyword sweeps kept as supplement; gamma ?search= is loose-token, rarely contributes net-new markets',
      // PR #170 diag: future-event-discovery surface promoted out of funnel
      // so the verification curl Marc specced can dot-access these directly.
      events_pulled:      (funnel.future_event_discovery && funnel.future_event_discovery.events_pulled)  || 0,
      events_matched:     (funnel.future_event_discovery && funnel.future_event_discovery.events_matched) || 0,
      speakers_hit:       (funnel.future_event_discovery && funnel.future_event_discovery.speakers_hit)   || [],
      ...
      dropped_editorial:        Number(droppedEditorial)        || 0,  // PR #170: off-thesis meme markets (GTA VI, Jesus return, new-album bets) — editorial scope, not data correctness
      ...
      markets_per_tier: {  // alias of tier_counts per PR #170 verification spec
        jumbo:  tiered.jumbo.length,
        large:  tiered.large.length,
        medium: tiered.medium.length,
        small:  tiered.small.length,
...
async function buildHeatmapResponse(opts = {}) {
  ...
  // Stage 0 (PR #170): editorial scope -- drops off-thesis meme markets
  // (GTA VI, "Jesus return", new-album bets) that the future-event
  // discovery pass picks up by mistake when their slug happens to contain
  // a speaker watchlist word. /mentions is mention-betting on political
  // and macro figures; pop-culture meme markets belong elsewhere.
  const onTopic = candidates.filter(_passesEditorialFilter);
  const droppedEditorial = candidatesBeforeFilter - onTopic.length;
  ...
  const live = onTopic.filter(_passesLivenessFilter);
  const droppedSettled = onTopic.length - live.length;
  ...
  const response = _composeResponse(filtered, funnel, candidatesBeforeFilter, droppedSettled, droppedResolved, droppedSettledBinaries, droppedEditorial);
