...
// recompute-flex).
const CODE_VERSION = '2026-05-14-vol-total-primary+liveness-filter';
...
// Liveness filter -- drops settled/inactive markets that surface via
// slug-probe (PR #157 dropped the closed flag filter at sourcing time
// to keep pending-activation markets visible, but settled markets share
// the same flag and were leaking through with stale prices).
//
// A market fails liveness if BOTH:
//   - yes_price is settled (== 0 or == 1, ie binary resolution price)
//   - volume_24h is zero (no recent trading -> not pre-activation either)
//
// Pre-activation markets typically hold at 50/50 or near-mid until
// trading opens, so they keep `yes_price` in (0, 1) and pass.
// Live markets either have volume_24h > 0 or a non-settled price, also pass.
// Markets with null yes_price (price not yet populated) are kept --
// surfacing them with "--" cents is better than hiding pending supply.
function _passesLivenessFilter(c) {
  const p = c.yes_price;
  const v24 = c.volume_24h || 0;
  const isSettledPrice = (p === 0 || p === 1);
  return !(isSettledPrice && v24 <= 0);
}
...
async function buildHeatmapResponse(opts = {}) {
  ...
  // Two-stage filter: liveness first (drops settled-and-quiet sub-markets
  // with stale 0/100 prices that PR #157's drop-closed-filter started
  // surfacing), then volume gate.
  const live = candidates.filter(_passesLivenessFilter);
  const droppedSettled = candidatesBeforeFilter - live.length;
  const filtered = live.filter(_passesVolumeFilter);
...
module.exports = {
  ...
  MIN_VOLUME_24H,
  MIN_VOLUME_TOTAL,
  _internals: { _passesVolumeFilter, _passesLivenessFilter, _bucketIntoTiers, _composeResponse, _sortDesc },
};
