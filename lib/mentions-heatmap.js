...
//
// PR 1 of the /mentions heat-map (claude/mentions-heatmap-sourcing-v1).
// Sourcing + endpoint only -- no UI. Sister PR claude/mentions-heatmap-ui-v1
// will consume the JSON this module produces.
//
// Pipeline (per 5-min cache refresh):
//   1. wordMarkets.getHeatmapCandidates() runs 4 keyword sweeps in parallel
//      (word_betting / statement_betting / speaker_action / event_decision),
//      attributes whitelist speakers, dedupes by conditionId.
...
function _composeResponse(filtered, funnel, totalCandidatesBeforeFilter) {
  ...
  return {
    ...
    sourcing: {
      patterns_shipped:   ['word_betting', 'statement_betting', 'speaker_action', 'event_decision'],
...
async function buildHeatmapResponse(opts = {}) {
  ...
  // [heatmap-source] structured per-pattern + tier diag log. Marc's spec
  // requires per-pattern market counts on push. Same line every refresh so
  // log-grep stays easy.
  console.log(
    '[heatmap-source]' +
