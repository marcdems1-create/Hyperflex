...
// recompute-flex).
const CODE_VERSION = '2026-05-15-future-event-sourcing-v2';
...
function _passesVolumeFilter(c) {
  // Hard noise filter: markets at extreme prices (0¢/99¢) with under $50k
  // total volume are settled or untraded junk (Korean elections, Hormuz warships).
  const p = c.yes_price;
  if (p != null && (p < 0.03 || p > 0.97)) return false;
  if ((c.volume_total || 0) < 50000) return false;
  return true;
}

function _heatmapScore(c) {
  const vol24 = Number(c.volume_24h)   || 0;
  const volT  = Number(c.volume_total) || 0;
  const p     = c.yes_price != null ? c.yes_price : 0.5;
  // Controversy bonus: markets priced near 50¢ are most interesting.
  // Score 1.0 at 50¢, fades to 0.1 at 0¢/100¢.
  const controversyScore = 1 - Math.abs(p - 0.5) * 2;
  return (vol24 * 3 + volT) * (controversyScore + 0.1);
}
...
// Sort by controversy-weighted volume: 24h activity × 3 + lifetime, scaled
// by how close to 50¢ (most controversial = most interesting). Markets
// at 0¢/99¢ with low volume are already filtered out upstream.
function _sortDesc(a, b) {
  return _heatmapScore(b) - _heatmapScore(a);
}
