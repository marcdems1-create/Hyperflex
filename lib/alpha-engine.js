...
function _attachWhaleData(markets) {
  ...
  // wc.data.whales is a flat array of position objects (one row per position),
  // NOT an array of whale objects with nested positions[]. Each element has:
  //   conditionId, size (shares), current_price, side/outcome, avg_price, proxyWallet, trader_rank
  const positions = (wc && wc.data && Array.isArray(wc.data.whales)) ? wc.data.whales : [];
  // Diagnostic: log sample conditionIds from both sides of the join so mismatches are visible
  const samplePos = positions.slice(0, 3).map(p => p.conditionId || p.condition_id || '(none)');
  console.log(`[alpha-engine] whale positions: ${positions.length}, sample conditionIds: ${JSON.stringify(samplePos)}`);
  const posMap = new Map();

  for (const pos of positions) {
    // Accept both camelCase and snake_case — Polymarket data API returns conditionId
    const cid = pos.conditionId || pos.condition_id;
    if (!cid) continue;
    // size is share count; current_price × size ≈ USD value — use $500 floor
    const usdValue = (pos.size || 0) * (pos.current_price || pos.avg_price || 0.5);
    if (usdValue < 500) continue;
    const e = posMap.get(cid) || { count: 0, yes: 0, no: 0, priceSum: 0 };
    e.count++;
    if ((pos.outcome || pos.side || '').toUpperCase() === 'NO') e.no++; else e.yes++;
    e.priceSum += Number(pos.avg_price || pos.avgPrice || pos.current_price || 0.5);
    posMap.set(cid, e);
  }

  // Diagnostic: log how many markets got a whale match
  let matched = 0;
  ...
  const sampleMkt = markets.slice(0, 2).map(m => m.conditionId || '(none)');
  console.log(`[alpha-engine] whale matches: ${matched}/${markets.length}, sample market conditionIds: ${JSON.stringify(sampleMkt)}`);
}
...
async function refresh() {
  try {
    ...
    // ── Assign category + normalize scores so categories compete equally ──────
    ...
    // Pre-assign category on every market
    for (const m of markets) m.category = getCategory(m.question);

    // Normalize: best market per category = 1000, rest scale proportionally.
    // This prevents high-volume sports from crowding out equally-sharp politics/crypto.
    const catMax = {};
    for (const m of markets) {
      if (!catMax[m.category] || m.score > catMax[m.category]) catMax[m.category] = m.score;
    }
    for (const m of markets) {
      const max = catMax[m.category] || 1;
      m.score_normalized = (m.score / max) * 1000;
      // Edge bonus on top of normalization so sharp signals still differentiate within+across categories
      m.score_normalized += (m.whale_count || 0) * 50;
      m.score_normalized += Math.max(0, ((m.edge_score || 0) - 5) * 30);
      m.score_normalized += m.days_to_close < 1 ? 100 : m.days_to_close < 3 ? 50 : 0;
      m.score_normalized += Math.abs(m.price_change_24h || 0) > 15 ? 75 : 0;
    }
    markets.sort((a, b) => b.score_normalized - a.score_normalized);

    // ── Hard category caps ────────────────────────────────────────────────────
    const catCount = { sports: 0, politics: 0, crypto: 0, other: 0 };
    markets = markets.filter(m => {
      if (catCount[m.category] >= CATEGORY_CAPS[m.category]) return false;
      catCount[m.category]++;
      return true;
    });
    markets.sort((a, b) => b.score_normalized - a.score_normalized);
