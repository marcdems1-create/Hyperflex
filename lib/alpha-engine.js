...
function score(m) {
  ...
  if (controversy < 0.35) return 0; // kills anything > 67¢ or < 33¢
...
function enrich(m) {
  ...
  if (why.length === 0) {
    why.push(`$${fmtVol(m.volume_24h)} traded in last 24h`);
    why.push(`${Math.round((m.yes_price || 0.5) * 100)}¢ YES — market open`);
  }
...
async function refresh() {
  try {
    ...
    // ── Category balance: cap sports at 30% of feed, inject non-sports if needed
    const total = markets.length;
    const sportsInFeed = markets.filter(m => {
      const q = (m.question || '').toLowerCase();
      return SPORT_PATTERNS.some(p => q.includes(p));
    }).length;

    if (total > 0 && sportsInFeed / total > 0.30) {
      // Too many sports — pull top non-sports from the full sorted list
      const NON_SPORT_KW = ['bitcoin', 'fed', 'trump', 'election', 'ai ', 'interest rate',
                             'crypto', 'ethereum', 'ukraine', 'israel', 'china', 'inflation',
                             'recession', 'president', 'senate', 'nuclear', 'tariff'];
      const inFeedSlugs = new Set(markets.map(m => m.market_slug));
      // Re-score all markets to find non-sport candidates not already in feed
      const nonSportPool = _rawList.filter(m => {
        if (inFeedSlugs.has(m.market_slug)) return false;
        if (m.score <= 0) return false;
        const q = (m.question || '').toLowerCase();
        const isSport = SPORT_PATTERNS.some(p => q.includes(p));
        if (isSport) return false;
        return NON_SPORT_KW.some(kw => q.includes(kw));
      }).sort((a, b) => b.score - a.score).slice(0, 5);

      // Remove lowest-scoring sport markets until ratio is ≤30%
      const sportIdxs = [];
      markets.forEach((m, i) => {
        const q = (m.question || '').toLowerCase();
        if (SPORT_PATTERNS.some(p => q.includes(p))) sportIdxs.push(i);
      });
      let removed = 0;
      while (nonSportPool.length > removed) {
        const curSports = sportsInFeed - removed;
        const curTotal  = total - removed + removed; // replacements keep count same
        if (curSports / markets.length <= 0.30) break;
        const idx = sportIdxs.pop(); // remove from end (lowest score, already sorted)
        if (idx == null) break;
        markets.splice(idx, 1, nonSportPool[removed]);
        removed++;
      }
    }
