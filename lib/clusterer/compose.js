...
async function upsertEvent(client, row, parsed, stance_disagreement) {
  ...
  await client.query(`
    ...
    do update set
      ...
      updated_at = now()
  `, [
    slug, title, row.speaker, row.event_type, event_at, 'Federal Reserve',
    blurb, 'past',
    ...
    row.comparison_yielded_no_divergence,
  ]);
}
...
async function run(opts = {}) {
  ...
  let sql = `select id, speaker, event_type, source_url, transcript_date from transcripts ${where} order by transcript_date desc`;
  if (limit > 0) {
    params.push(limit);
