...
async function run() {
  ...
  // Self-diagnosing breakdowns. `rows_written: 30` looks reasonable until you
  // realize it should be ~speakers × 30 — the speaker_breakdown surfaces a
  // single-speaker corpus immediately, and stance_breakdown shows whether
  // the rule-based classifier is actually producing non-neutral output.
  const speakerCounts = new Map();
  const stanceCounts = { hawkish: 0, dovish: 0, neutral: 0, insufficient_data: 0 };
  for (const r of rows) {
    speakerCounts.set(r.speaker, (speakerCounts.get(r.speaker) || 0) + 1);
    if (r.stance in stanceCounts) stanceCounts[r.stance]++;
  }
  const speakerBreakdown = Object.fromEntries(
