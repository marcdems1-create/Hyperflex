...
//   settled_events < 5   → qualifies=false, UI shows a low-sample
//                          indicator, user NOT on public leaderboard.
//                          Score still computes from the first settled event.
//
// Output shape matches sports-flex-score for cron writer reuse.

const THRESHOLDS = {
  // PR 2026-05-12: lowered 25 → 5. Per Marc's product call, the score is
  // now the canonical signal from the first trade; below this threshold
  // a "low sample" indicator surfaces on profile but the score still
  // shows. `qualifies=false` only governs public-leaderboard ranking
  // eligibility now, not score visibility.
  BUILDING_MAX_SETTLED: 5,    // below this → low-sample indicator, NOT on public leaderboard
  ACCURACY_CAP_SETTLED: 10,   // below this → raw WR capped at 60% inside formula (small-sample guard stays)
...
// The qualification gate (n<5) still keeps these tiny samples off the
// public leaderboard.
...
// Output:
//   {
//     score,                           // 0..100, or null when settled=0
//     qualifies,                       // true when settled_events ≥ 5 (leaderboard gate)
...
//     reasons,                         // ['need 5 settled events (have 2)', ...]
//   }
...
function computeFlexScore(stats) {
  ...
  // Score is canonical from the first settled event. `qualifies` governs
  // public-leaderboard ranking eligibility (still gated at 5 settled),
  // NOT score visibility. Surfaces that render the score show it at any
  // sample size; the `reasons` array is the hook for a "low sample"
  // indicator. Only return null when there's truly no signal (settled=0).
  const score = settled > 0
...
// Tier ladder — pure function of score. Tier derives from score alone;
// `qualifies` is no longer a gate on tier display (Marc 2026-05-12: FLEX
// SCORE is the score of truth, must show consistently). Only return
// 'Unranked' when there's literally no signal (score=null, settled=0).
// Callers that still pass `qualifies` as a 2nd arg get ignored — kept
// in the signature for backwards-compat with existing call sites.
function tierForScore(score, _qualifies) {
