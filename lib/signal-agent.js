...
Be decisive. edge_rating >= 6 means worth_surfacing = true. Rate honestly.`;
  ...
  async _persist(signal) {
    if (!this.pool) return;
    try {
      // Conflict target must match agent_signals_dedup_utc_idx's expression
      // exactly. NOTE: before the index expression was made IMMUTABLE-safe,
      // the index never existed and this ON CONFLICT made EVERY persist throw
      // ("no unique or exclusion constraint matching") — signals were
      // cache-only across restarts.
      await this.pool.query(`
        ...
        ON CONFLICT (market_slug, side, ((fired_at AT TIME ZONE 'UTC')::date)) DO UPDATE
  ...
  async _ensureTable() {
    if (!this.pool) return;
    // Statements run SEPARATELY — the old single multi-statement batch ran in
    // one implicit transaction, so the dedup-index failure rolled back the
    // entire ensure on every boot.
    try {
      await this.pool.query(`
    ...
    try {
      await this.pool.query(`CREATE INDEX IF NOT EXISTS agent_signals_fired_at_idx ON agent_signals(fired_at DESC)`);
    } catch (err) {
      console.warn('[signal-agent] fired_at index error:', err.message);
    }
    try {
      // Clear duplicate (slug, side, UTC-day) rows from the era when the
      // dedup index couldn't build, keeping the newest — otherwise the
      // unique index still can't build. No-op on a clean table.
      await this.pool.query(`
        DELETE FROM agent_signals a USING agent_signals b
        WHERE a.id < b.id
          AND a.market_slug = b.market_slug
          AND a.side IS NOT DISTINCT FROM b.side
          AND ((a.fired_at AT TIME ZONE 'UTC')::date) = ((b.fired_at AT TIME ZONE 'UTC')::date)`);
      // fired_at::date on a timestamptz depends on the session TimeZone, so
      // Postgres rejects it in an index expression ("functions in index
      // expression must be marked IMMUTABLE"). Pinning the zone makes the
      // cast deterministic: (fired_at AT TIME ZONE 'UTC')::date.
      await this.pool.query(`
        CREATE UNIQUE INDEX IF NOT EXISTS agent_signals_dedup_utc_idx
          ON agent_signals(market_slug, side, ((fired_at AT TIME ZONE 'UTC')::date))`);
    } catch (err) {
      console.warn('[signal-agent] dedup index error:', err.message);
    }
  }
}
