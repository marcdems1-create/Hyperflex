...
const SMOKE_FIXTURES = [
  {
    name: 'multi-outcome language event (Powell May presser)',
    slug: 'what-will-powell-say-during-may-press-conference',
  ...
  // code path or have caught a real bug. Powell-language exercises:
  //   - Gamma /events (vs /markets) lookup
  //   - Multi-outcome path (one parent, N binary sub-markets)
  //   - Active-only filter (the bug that prompted Phase 1)
  //   - CLOB best-ask refresh across many tokens
  // Rotated 2026-05-07 from `what-price-will-wti-hit-in-april-2026`,
  // which resolved Apr 30 and was leaking 39/40 closed:true markets +
  // 0/40 live CLOB books on every boot. Pick fixtures with months of
  // liquid runway — Fed presser language markets are guaranteed live
  // until the meeting + a settlement window after.
];
 ...
 * @param {string}  [opts.endDateMin]  ISO date string
 * @param {string}  [opts.endDateMax]  ISO date string
...
async function fetchPolymarketEvents(opts = {}) {
  ...
  const { tag, active = true, endDateMin, endDateMax, limit = 50 } = opts;
