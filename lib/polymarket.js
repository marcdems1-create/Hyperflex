 ...
 * @param {string}  [opts.end_date_min]  ISO date string — gamma snake_case param
 * @param {string}  [opts.end_date_max]  ISO date string — gamma snake_case param
 ...
 *
 * IMPORTANT: opts intentionally use gamma's snake_case names. Empirically
 * verified 2026-05-15: camelCase URL params (`endDateMin`, `endDateMax`) are
 * SILENTLY IGNORED by gamma — returns unfiltered results (e.g. 2021 NFL
 * events). Keeping the opt names identical to the URL params eliminates the
 * translation layer where a future caller could drift back to camelCase.
 */
async function fetchPolymarketEvents(opts = {}) {
  ...
  const { tag, active = true, closed, end_date_min, end_date_max, order, ascending, limit = 50 } = opts;
