...
const REDEMPTION_TIERS = [
  { points: 3000,  credit_cents: 500,  label: '$5 subscription credit' },
  { points: 8000,  credit_cents: 1500, label: '$15 subscription credit' },
  { points: 15000, credit_cents: 2900, label: '1 month Pro free' },
  { points: 45000, credit_cents: 9900, label: '1 month Premium free' },
];

// Calculate reward points for a set of new synced positions
function calcSyncRewardPoints(newPositions) {
  const byPlatform = { polymarket: 0, kalshi: 0, manifold: 0 };
  for (const p of (newPositions || [])) {
    if (byPlatform[p.platform] !== undefined) byPlatform[p.platform]++;
  }
  return {
    total: byPlatform.polymarket * REWARD_POINTS.sync_polymarket +
...
// Check if a redemption tier is affordable
function canRedeemTier(pointsBalance, tierIndex) {
  const tier = REDEMPTION_TIERS[tierIndex];
  if (!tier) return false;
  return pointsBalance >= tier.points;
}

// ── Polymarket Referral Tag ───────────────────────────────
// Appends ?via=CODE to any polymarket.com URL.
// Skips API subdomains (data-api, clob, gamma-api).
// Reads from POLYMARKET_REF env var (server) or accepts code directly.
function polyRef(url, refCode) {
  const code = refCode || (typeof process !== 'undefined' && process.env && process.env.POLYMARKET_REF) || '';
  if (!code || !url) return url || '';
  const s = String(url);
  // Only tag polymarket.com — skip API subdomains and non-polymarket URLs
  if (!s.includes('polymarket.com')) return s;
  if (s.includes('data-api.polymarket.com') || s.includes('clob.polymarket.com') || s.includes('gamma-api.polymarket.com') || s.includes('docs.polymarket.com')) return s;
  // Don't double-tag
  if (s.includes('via=')) return s;
  // Append ?via= or &via=
