...
async function getHotMarketsCarousel(opts = {}, deps) {
  ...
  deps = deps || {};
  const limit = Math.max(1, Math.min(80, Number(opts.limit) || DEFAULT_LIMIT));
  const now = Date.now();
...
const TOPIC_RULES = [
  { topic: 'US Politics',   keywords: ['trump','biden','congress','senate','election','republican','democrat','white house','supreme court','president','harris','desantis','governor','house rep','vote','ballot','campaign','incumbent','midterm','filibuster','veto','inauguration'] },
  { topic: 'Crypto',        keywords: ['bitcoin','btc','ethereum','eth','crypto','defi','nft','solana','coinbase','binance','stablecoin','altcoin','blockchain','web3','doge','xrp','bnb','usdc','usdt','polygon','avalanche','chainlink','cardano','polkadot'] },
  { topic: 'Sports',        keywords: ['nba','nfl','mlb','nhl','fifa','ufc','tennis','soccer','championship','super bowl','world cup','playoffs','league','quarterback','touchdown','mvp','draft','trade deadline','roster','coach','stadium','match','tournament','formula 1','f1','olympics','wimbledon','masters','nascar'] },
  { topic: 'World Events',  keywords: ['russia','ukraine','china','iran','israel','nato','war','ceasefire','peace','nuclear','taiwan','gaza','hamas','hezbollah','sanctions','treaty','diplomacy','coup','refugee','humanitarian','un security','g20','g7','brics','un resolution'] },
  { topic: 'Economics',     keywords: ['fed','inflation','recession','gdp','rate cut','interest rate','jobs report','unemployment','market crash','stock','s&p','dow jones','cpi','ppi','fomc','jerome powell','treasury','bond yield','dollar','trade deficit','ipo','earnings','fiscal','monetary'] },
  { topic: 'AI & Tech',     keywords: ['ai','openai','gpt','anthropic','nvidia','apple','google','meta','microsoft','tech','model','claude','gemini','llm','machine learning','artificial intelligence','chatgpt','deepmind','agi','elon musk','tesla','spacex','starlink','regulation','antitrust'] },
];

function classifyTopic(market) {
  const text = (
    (market.market_question || market.question || market.title || '') + ' ' +
    (market.event_title || '') + ' ' +
    (market.category || '') + ' ' +
    (Array.isArray(market.news_citations) ? market.news_citations.map(n => n.headline || '').join(' ') : '')
  ).toLowerCase();
